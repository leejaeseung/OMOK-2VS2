...
public class ClientEventHandler implements CMAppEventHandler {
	...
	private void processDummyEvent(CMEvent cme) throws Exception {
		...
		case "gamestartB":
			...
			m_client.setGui(new GUI(m_client.getId(), "오목", m_client, m_client.getRoom().rName, "black", m_client.getTchat()));
		...
		case "gamestartW":
			...
			m_client.setGui(new GUI(m_client.getId(), "오목", m_client, m_client.getRoom().rName, "white", m_client.getTchat()));
			m_client.getTchat().setBounds(200, 200, 400, 300);
			m_client.getTchat().setVisible(true);
			break;
		...
		case "gamestartD":
			...
			m_client.setGui(new GUI(m_client.getId(), "오목", m_client, m_client.getRoom().rName, "watch", m_client.getTchat()));
			m_client.getTchat().setBounds(200, 200, 400, 300);
			m_client.getTchat().setVisible(true);
			break;
